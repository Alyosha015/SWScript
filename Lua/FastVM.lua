--[[ Stack Frame format
 1: (string) stack frame name
 2: (table) program bytecode
 3: (table) local variable values
 4: (table) names of stackframes where locals for their corresponding upvalues are located
 5: (table) indexes of locals in their stackframes for their corresponding upvalues
 6: (boolean) true if lua function
 7: (number) program counter
 8: (table) stores upvalues directly (used instead of modifying/getting locals from other stackframe if said frame is not on callstack)
 9: (number) stores number of local variables (used when making copy of stackframe, getting length of local table doesn't work because there's a chance one of the locals is set to nil)
10: (number) number of values function will return. (used in return instruction, in the case the function returned less values in reality extra nil's will be pushed onto the stack).
]]
